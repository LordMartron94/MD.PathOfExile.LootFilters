...
class StyleRenderer(Renderer):
    ...
    def _render_basic_colors(self, lines: List[str], style: Rule.style.__class__) -> None:
    ...
    def _render_optional_icon(self, lines: List[str], style: Rule.style.__class__) -> None:
        icon = style.minimap_icon
        if icon:
            self._adder(lines, "MinimapIcon", icon.size, icon.colour, icon.shape)

    def _render_effect(self, lines: List[str], style: Rule.style.__class__) -> None:
        pe = style.play_effect
        if pe:
            token = pe.colour + ("Temp" if pe.temp else "")
            self._adder(lines, "PlayEffect", token)

    def _render_sounds(self, lines: List[str], style: Rule.style.__class__) -> None:
    ...
    def _render_drop_toggles(self, lines: List[str], style: Rule.style.__class__) -> None:
