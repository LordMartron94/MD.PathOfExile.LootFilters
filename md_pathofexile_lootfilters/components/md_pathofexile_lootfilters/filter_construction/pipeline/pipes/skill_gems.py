...
class AddSkillGems(IPipe):
    def __init__(
        ...
        self._section_description = "Tiers the skill gems based on economy rarity data."
    ...
    def _prepare_economy_df(data: FilterConstructionPipelineContext) -> pd.DataFrame:
        """
        Load and clean economy data for skill gems, keeping raw rarity (1-12).
        """
        df = data.skill_gems_data.copy()
        df = df.rename(columns={"Base Type": "BaseType", "Rarity": "Rarity (1-12)"})
        df["Rarity (1-12)"] = df["Rarity (1-12)"].astype(int)

        # Filter to only valid skill gems
        df = df[df["BaseType"].isin(data.valid_base_types_unique_and_gem)]

        return df[["BaseType", "Rarity (1-12)"]]

    def _get_rules(self, data: FilterConstructionPipelineContext) -> List[Rule]:
        # Prepare economy-only DataFrame
        econ_df = self._prepare_economy_df(data)

        final_df = econ_df.astype(object).replace({pd.NA: None})
        final_df.to_csv(DATA_DIR / "gems_economy_stats.csv", index=False)

        # Apply tier rules using raw rarity 1-12 accessor
        rules: List[Rule] = []
