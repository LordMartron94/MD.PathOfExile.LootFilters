...
class HelmetProgressionBuilder:
    ...
    def get_progression_rules(self, data: FilterConstructionPipelineContext) -> List[Rule]:
        _item_progression: List[ItemProgressionItem] = get_item_progression_for_category(BaseTypeCategory.helmets, data.base_type_data)
        item_progression_config = get_default_item_progression_config(data, self._rule_factory, self._condition_factory, ArmorTypeClass.Helmets)
